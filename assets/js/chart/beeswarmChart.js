...
// code comes from https://observablehq.com/@d3/beeswarm#BeeswarmChart
//Don't touch it too much
//It has been modified to display colours (fill option) and tooltips

export function BeeswarmChart(data, {
    ...
    const T_TITLE = d3.map(data, tooltipTitle).map(tooltipTitle => tooltipTitle == null ? NaN : tooltipTitle);
    const T_BODY = d3.map(data, tooltipBody).map(tooltipBody => tooltipBody == null ? NaN : tooltipBody);
    const FILL = d3.map(data, fill).map(fill => fill == null ? NaN : fill);
    ...
    const dot = svg.append("g")
        ...
        .attr('data-bs-content', i => T_BODY[i])
